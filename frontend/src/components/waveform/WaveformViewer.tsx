...
//     const signalHeight = 48 * dpr;
//     const signalSpacing = 8 * dpr;
//     const rows = Math.max(1, waves.length);
//     const desiredHeight = rows * (signalHeight + signalSpacing) + margin.top + margin.bottom;
...
//     const N = timestamps.length;
//     const indexToX = (i: number): number => {
//       const xFull = margin.left + (i * contentW) / N;
//       return xFull - safeOffset;
//     };

//     // Signal names panel background
...
//     ctx.lineWidth = 1 * dpr;
//     const gridEvery = Math.max(1, Math.floor(N / 10));
//     for (let i = 0; i <= N; i += gridEvery) {
//       const x = indexToX(i);
//       if (x < margin.left || x > margin.left + plotW) continue;
//       ctx.beginPath();
...
//       ctx.textAlign = "center";
//       const time = timestamps[Math.min(i, N - 1)];
//       ctx.fillText(`${time}ns`, x, canvas.height - 10 * dpr);
//     }

//     // Draw each signal
//     waves.forEach((sig, row) => {
//       const yTop = margin.top + row * (signalHeight + signalSpacing);
//       const yHigh = yTop + 10 * dpr;
//       const yLow = yTop + signalHeight - 10 * dpr;
//       const mid = (yHigh + yLow) / 2;
...
//       ctx.lineWidth = 1 * dpr;
//       ctx.beginPath();
//       ctx.moveTo(margin.left, yTop + signalHeight + signalSpacing / 2);
//       ctx.lineTo(canvas.width - margin.right, yTop + signalHeight + signalSpacing / 2);
//       ctx.stroke();
...
//       ctx.textAlign = "right";
//       ctx.fillText(sig.name, margin.left - 12 * dpr, yTop + signalHeight / 2 + 4 * dpr);
...
//         const x = indexToX(i);
//         const nextX = indexToX(i + 1);
//         if (nextX < margin.left || x > margin.left + plotW) continue;
...
//         const centerX = (indexToX(i) + indexToX(i + 1)) / 2;
//         if (centerX < margin.left || centerX > margin.left + plotW) continue;
//         ctx.fillText(pts[i], centerX, yTop + signalHeight / 2 + 16 * dpr);
//       }
//     });
...
//         // Compute time
//         const rel = x - margin.left + safeOffset;
//         const frac = clamp(rel / contentW, 0, 0.999999);
//         const idx = Math.floor(frac * N);
//         const time = timestamps[idx];
//         setCursorTime(time);

//         // Cursor label with rounded box
//         const label = `${time}ns`;
//         const pad = 6 * dpr;
//         ctx.font = `bold ${11 * dpr}px 'JetBrains Mono', monospace`;
//         const textW = ctx.measureText(label).width;
//         const boxX = clamp(x - textW / 2 - pad, margin.left, margin.left + plotW - textW - pad * 2);
//         const boxY = Math.max(4 * dpr, margin.top - 24 * dpr);
//         const boxH = 18 * dpr;
//         const radius = 4 * dpr;
...
//         ctx.beginPath();
//         ctx.roundRect(boxX, boxY, textW + pad * 2, boxH, radius);
//         ctx.fill();

//         ctx.fillStyle = "#ffffff";
//         ctx.textAlign = "left";
//         ctx.fillText(label, boxX + pad, boxY + 13 * dpr);
//       }
//     } else {
//       setCursorTime(null);
//     }
...
//   // Interaction handlers (unchanged functionality)
//   const handleZoom = (factor: number, centerCssX?: number) => {
//     const canvas = canvasRef.current;
//     if (!canvas) return;
//     const dpr = window.devicePixelRatio || 1;
...
//     const centerCanvasPx = (centerCss - canvas.getBoundingClientRect().left) * dpr;
//     const viewX = clamp(centerCanvasPx - marginLeft, 0, plotW);
//     const worldX = offset + viewX;
//     const newZoom = clamp(zoom * factor, 1, 32);
//     const newContentW = plotW * newZoom;
//     let newOffset = worldX - viewX;
//     newOffset = clamp(newOffset, 0, Math.max(0, newContentW - plotW));
...
//     if (isPanning && panLastX.current !== null) {
//       const dxCss = e.clientX - panLastX.current;
//       const dpr = window.devicePixelRatio || 1;
//       const dx = dxCss * dpr;
//       const marginLeft = 120 * dpr;
//       const marginRight = 20 * dpr;
...
//       const contentW = plotW * zoom;
//       const maxOffset = Math.max(0, contentW - plotW);
//       setOffset((o) => clamp(o - dx, 0, maxOffset));
//       panLastX.current = e.clientX;
//     } else {
//       setCursorX(e.clientX - rect.left);
//     }
...
//       }}
//     >
//       <TimelineIcon sx={{ fontSize: 48, color: "#475569", mb: 2 }} />
...
//             }}
//           >
//             <TimelineIcon sx={{ fontSize: 18, color: "#fff" }} />
//           </Box>
//           <Box>
...
//             </Typography>
//             <Typography variant="caption" sx={{ color: "#64748b" }}>
//               {waves.length} signals • {timestamps.length > 0 ? timestamps[timestamps.length - 1] : 0}ns
//             </Typography>
//           </Box>
//         </Stack>
...
//               <IconButton size="small" onClick={zoomOut} sx={{ color: "#94a3b8", "&:hover": { bgcolor: "#475569" } }}>
...
//               <IconButton size="small" onClick={zoomIn} sx={{ color: "#94a3b8", "&:hover": { bgcolor: "#475569" } }}>
...
//             <IconButton size="small" onClick={resetView} sx={{ color: "#94a3b8", "&:hover": { bgcolor: "#475569" } }}>
//               <RefreshIcon sx={{ fontSize: 18 }} />
//             </IconButton>
//           </Tooltip>
//           <Tooltip title="Fullscreen">
//             <IconButton size="small" sx={{ color: "#94a3b8", "&:hover": { bgcolor: "#475569" } }}>
...
//       >
//         <Typography variant="caption" sx={{ color: "#64748b" }}>
//           Tip: <b>Ctrl + Scroll</b> to zoom • <b>Drag</b> to pan
//         </Typography>
//         <Stack direction="row" spacing={3}>
//           <Typography variant="caption" sx={{ color: "#64748b" }}>
//             Scale: <span style={{ color: "#10b981" }}>{vcdData.scale || "1ns"}</span>
//           </Typography>
//           {cursorTime !== null && (
//             <Typography variant="caption" sx={{ color: "#64748b" }}>
//               Cursor: <span style={{ color: "#22d3ee" }}>{cursorTime}ns</span>
//             </Typography>
//           )}
//         </Stack>
//       </Box>
...
export default function WaveformViewer({ vcdData }: any) {

  const signals = vcdData?.signal?.map((s:any)=>s.name) || [];

  const [visibleSignals, setVisibleSignals] = useState<string[]>(signals);
  ...
  return (
    ...
    <div className="flex h-[80vh] bg-slate-950">
